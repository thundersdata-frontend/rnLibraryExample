import React, {useState} from 'react';
import {ScrollView, Text} from 'react-native';
import { WhiteSpace, Button} from '@td-design/react-native';
import { Container } from '@/components/Container';
import { Image } from 'expo-image';

import base64Img from '@/components/fields';

const baseUri =
  'https://images.pexels.com/photos/1702238/pexels-photo-1702238.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=100';
export default () => {
  const [uri, setUri] = useState(baseUri);

  return (
    <Container>
      <ScrollView style={{flex: 1}}>
        {/* <Text>基本:</Text>
        <Image
          source={{
            uri,
          }}
          style={{ width: 100, height: 100 }}
          preview
        />
        <WhiteSpace />
        <Button title="重新请求" onPress={() => setUri(baseUri + `?bust=${Math.random().toString()}`)} /> */}
        <WhiteSpace />
        <Text>本地jpg:</Text>
        <Image
          source={require('@/images/fieldsJpg.jpg')}
          style={{width: 300, height: 300}}
        />
        <WhiteSpace />
        <Text>本地png:</Text>
        <Image
          source={require('@/images/logo.png')}
          style={{width: 300, height: 300}}
        />
        <WhiteSpace />
        <Text>本地gif:</Text>
        <Image
          source={require('@/images/jellyfish.gif')}
          style={{width: 300, height: 300}}
        />
        <WhiteSpace />
        <Text>本地webp:</Text>
        <Image
          source={require('@/images/fieldsWebp.webp')}
          style={{width: 300, height: 300}}
        />
        <WhiteSpace />
        <Text>base64:</Text>
        <Image source={{uri: base64Img}} style={{width: 300, height: 300}} />
        <WhiteSpace />
      </ScrollView>
    </Container>
  );
};
